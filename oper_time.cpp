...
oper_time_t::oper_time_t(eeprom_safe_map_t<combination_t, uint32_t> *ap_eeprom_safe_map,
                         int a_save_time_period_min, const combination_t &a_start_combination, bool a_enable) : mp_eeprom_safe_map(ap_eeprom_safe_map),
        ...
        case status_t::find_least_time: {
            if (mp_eeprom_safe_map->ready()) {
                if (m_key_index < mp_eeprom_safe_map->get_keys_count() - 1) {
                    ...
                    mp_eeprom_safe_map->replace_key(mp_eeprom_safe_map->get_key(m_least_time_index), m_combination, m_combination_time);
                    m_status = status_t::wait_action_end;
                }
            }
        } break;
